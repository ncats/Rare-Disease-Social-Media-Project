...
class AnalyzeTopics:
    ...
    def __init__(self,
        ...
        if path is None:
            results_path = get_data_path('results')
            self.path = Path(results_path, name)
        else:
            self.path = path
        ...
        if self.model_type == 'LDA':
            ...
            dump_json(coherence_values_per_topic,
                      self.path,
                      f'{name}_coherence_values_per_topic_LDA')
            ...
            dump_json(word_score_dict,
                      self.path,
                      f'{name}_LDA_word_score_dict')
            ...
            dump_json(docs_per_topic,
                      self.path,
                      f'{name}_LDA_docs_per_topic')
            # Creates the document distribution from the clustered documents.
            create_distplot(docs_per_topic, self.name, self.path)
            ...
            dump_json(tfidf_word_score_dict,
                      self.path,
            ...
            dump_json(tfidf_coherence_values_per_topic,
                      self.path,
                      f'{self.name}_LDA_tfidf_coherence_values_per_topic')
            ...
            dump_json(topic_sizes,
                      self.path,
                      f'{name}_topic_sizes_LDA')
            ...
            dump_json(topic_sizes_dict,
                      self.path,
                      f'{name}_topic_sizes_Top2Vec')
            # Creates a distribtion plot of number of documents for each topic.
            create_distplot(topic_sizes, f'{self.name} Top2Vec', self.path)
            ...
            dump_json(coherence_values_per_topic,
                      self.path,
                      f'{name}_coherence_values_per_topic_Top2Vec')
            ...
            dump_json(word_score_dict,
                      self.path,
                      f'{name}_word_score_dict_Top2Vec')
            ...
            for topic in topic_nums:
                if topic_sizes[topic] >= 10:
                    create_wordcloud(word_score_dict[topic],
                                    ...
                                    topic_sizes[topic],
                                    'Topic Word',
                                    f'{self.name}_Top2Vec',
                                    self.path)
            # Creates the coherence distribution plot of coherence values for each topic with the
            # dashed line showing the mean coherence value.
            create_coherence_distplot(coherence_values_per_topic, f'{self.name} Top2Vec', self.path)
